...
\el{I am thinking about removing the $1/m$ from the definition. Pro: Its cleaner. Con: It's less like the Rademacher complexity. What do you think?}
\begin{definition}
...
\]
\end{definition}
The discrepancy of $f$ or $\F$ without a reference set $\{x_1,\ldots,x_m\}$ is the upper bound on any such set.
...
Having a low Radamacher complexity means that if we can optimize over a sample of roughly half the examples at random (each w.p.\ $1/2$).
...
Now consider $F_1(q) = F(q) + E(q)   = \sum_{i | \sigma_i=1} 2 f(x_i,q)$ and similarly $F_{-1}(q) = F(q) - E(q)   = \sum_{i | \sigma_i=-1} 2 f(x_i,q)$. We have that both $F_1(q)$ and $F_{-1}(q)$ are approximations for $F(q)$ obtained by coresets of item weights of $2$ and an error at most $|F_1(q) - F(q)| =  |E(q)|$.
...
Typically we can show that $K_m = O(c/m)$ or $K_m = c/\sqrt{m}$ for some $c$ which does not depend on $m$.

\begin{fact}
Let $\F$ be a function family with a corresponding discrepancy $K_m(f) = O(c/m)$. Let $F(q) = \sum_{i=1}^{n}f(x_i, q)$. There exists a coreset for $F$ of size $O(c/\eps)$ whose error is at most $\eps n$.
\end{fact}
\begin{fact}
Let $\F$ be a function family with a corresponding discrepancy $K_m(f) = O(c/\sqrt{m})$. Let $F(q) = \sum_{i=1}^{n}f(x_i, q)$. There exists a coreset for $F$ of size $O(c^2/\eps^2)$ whose error is at most $\eps n$.
\end{fact}
...
For any function family $\F$ with a corresponding discrepancy $K_m = O(c/m)$ there exists an fully-mergeable streaming coreset randomized algorithm of size $O\left(c\log^2\log(n/\delta)/\eps\right)$ whose error is at most $\eps n$ with probability at least $1-\delta$. \el{for a single query?}
\end{theorem}
...
For any function family $\F$ with a corresponding discrepancy $K_m = O(c/\sqrt{m})$ there exists a fully-mergeable streaming coreset randomized algorithm of size $O\left(c^2\log^3\log(n/\delta) /\eps^2\right)$ whose error is at most $\eps n$ with probability at least $1-\delta$. \el{for a single query?}
\end{theorem}

The algorithms must be given $\eps, \delta$ as input but do not need to know the length of the stream, $n$, in advance. 
The proofs of the above two theorems are quite technical and delegated to the appendix. Most of the ideas are already present in the recent breakthrough work on streaming quantiles \cite{DBLP:conf/focs/KarninLL16}. Generalizing the construction at  \cite{DBLP:conf/focs/KarninLL16} required resolving some added complications. The main ideas are as follows. 
...
The output stream contains roughly half the items from the input stream with double the weight. 
It does so by keeping a buffer of a certain capacity $k$. When a new item is inserted into the compactor it is added to its buffer. 
...
We begin with showing some basic properties of deterministic compactor, which always output the smaller of the two set  $\{ x_i | \sigma_i = 1\}$ or  $\{ x_i | \sigma_i = -1\}$. Consider a single compaction and let $\tilde F$ denote the function evaluated on the output set.
Let $E(q) = \sum_i \sigma_i f(x_i,q)$ for the signs $\sigma$ computed by the algorithm above. 
$$\tilde F(q) = \sum_{i ,\; \sigma_i=1} 2f(x_i, q) = \sum_{i} f(x_i, q) +  \sum_{i} \sigma_i f(x_i, q) = F(q) + E(q)$$
$$\tilde F(q) = \sum_{i ,\; \sigma_i=-1} 2f(x_i, q) = \sum_{i} f(x_i, q) - \sum_{i} \sigma_i f(x_i, q) = F(q) - E(q)$$
Note that $|\tilde F(q) - F(q)| = |E(q)| \le \max_q |\sum_i \sigma_i f(x_i,q)| = O(mK_m)$. 
...
These facts alone already allow us to prove Theorem~\ref{thm:streaming11}. 
...
Since we have $H \le log(\eps n)$ such compactors the overall space complexity is $c\log^2(\eps n/c)/\eps$.
\end{proof}

This algorithm is a direct extension the well know MRL algorithm \cite{MRL} for quantile sketching. 
