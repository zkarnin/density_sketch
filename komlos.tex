...
\section{Universal Coresets for Analytical Functions}

We provide a coreset suitable for analytical functions of the inner product $\ip{q,x}$. The idea is to find a set of signs that simultaneously balance $\ip{q,x}^k$ for all powers $k$ and unit vectors $q$. By controlling all powers of $\ip{q,x}$ we control any sum of these powers. It follows that this coreset can be used to control, for example, the logistic loss function $L(q,x) = \log(1+\exp(\ip{q,x}))$. 

We start some notation and trivial properties. For a vector $q \in \R^d$ let $q^{\otimes k}$ to represent the $k$-dimensional tensor obtained from the outer product of $q$ with itself in all the dimensions. For a $k$ dimensional tensor with $d^k$ entries $X$ we consider the measure
$\|X\| = \max_{q \in \R^d, \|q\|=1} \left| \langle X, q^{\otimes k}\rangle \right|$.
\begin{lemma}
$\|X\|$ is a norm
\end{lemma}
\begin{proof}
Notice that for any $X \neq 0$, $\ip{X, q^{\otimes}}$ is a non-zero polynomial in $q$. It follows that there must be $q$ for which it's value is non-zero. This means that $\|X\|=0$ iff $X=0$. For a scalar $a$, we clearly have by definition that
$$ \|aX\| = |a|\|X\| $$
Lastly, by the max definition we clearly have
$$ \|X+Y\| =  \max_{q \in \R^d, \|q\|=1} \left| \langle X+Y, q^{\otimes k}\rangle \right| \leq \max_{q \in \R^d, \|q\|=1} \left| \langle X, q^{\otimes k}\rangle \right| + \max_{q \in \R^d, \|q\|=1} \left| \langle Y, q^{\otimes k}\rangle \right| = \|X\| + \|Y\|$$
\end{proof}
...
For any set of vectors $x_i \in \R^d$ there exist a set of signs $\sigma_i$ such that for all $k$ simultaneously $\|\sum_i \sigma_i x_i^{\otimes k}\| \le O(\sqrt{d k\log^{3}{k}})$ (the $3$ power of the term $\log(k)$ can be reduced to any constant power larger than $2$). 
\end{lemma}
\begin{proof}
The proof will use Banaszczyk's theorem. 
Let $\mathcal K$ be a convex body in Euclidean space with Gaussian measure at least 1/2 ($\Pr[g \in \mathcal K] \ge 1/2$ where $g$ is i.i.d.\ Gaussian).
Let $x_1,\ldots,x_n$ be vectors with $\|x_i\|_2 =O(1)$. 
...
We define $\|\psi\|_T = \max_k \|\psi_k\|/\sqrt{\log(k)}$. 
Here, $\|\cdot\|$ is the special spectral norm defined in the beginning of the section 
%$$\|\psi\| = \max_{q, \|q\|=1} \langle   \psi_k,q^{\otimes k}\rangle  = \max_{q, \|q\|=1} \sum_{i_1,\ldots,i_k=1}^d \psi_k(i_1,\ldots,i_k)\Pi_{k'=1}^{k}q(i_{k'})$$
%
...
Considering a random i.i.d.\ Gaussian Tensor $g_k \in \R^{d^k}$. 
A trivial modification of Theorem 1 from \cite{tomioka2014spectral} shows that $\Pr[\|g_k\| \ge c\sqrt{d\log(k)}] \le 1/10k^2$ for some constant $c$. The only change needed in the proof is the size of the epsilon net which changes from $(2\log(3/2)/k)^{kd}$ for \cite{tomioka2014spectral} to $(2\log(3/2)/k)^d$, where the reason we require a net over a smaller space is due to us bounding the inner product with a rank one tensor rather than rank $k$. Union bounding on all values of $k$ we get $\sum_k 1/10k^2 \le 1/2$ which shows $g = [g_1, \operatorname{flat}(g_2), \operatorname{flat}(g_3), \ldots]$ belongs to $\mathcal K$ with probability al least $1/2$. The function $\operatorname{flat}(X)$ simply strings the entry values of the tensor $X$ into a vector.
...
Finally, note that for $\|x\|_2 \le 1$ we have $\|\psi(x)\|_2 = (\sum_k  1/k\log^2(k))^{1/2} = O(1)$.
...
Since $\psi_k = \sum_i \sigma_i x_i^{\otimes k}/\sqrt{k \log^2{k}}$ we get that $\max_k \|\sum_i \sigma_i  x_i^{\otimes k}\| \le \sqrt{d k \log^{3}(k)}$


\begin{lemma} \label{lem:komlos anl}
Let $f$ be a function of $\langle x,q\rangle$ such that $f(\langle x,q\rangle) = \sum_k \alpha_k \langle x,q\rangle^k$ is its Taylor expansion. The Komlos Complexity of the Family of functions $f_q(x) = f(\ip{q,x})$, indexed by $\|q\|_2 \leq 1$, is bounded by
\[
K_m = \min_\sigma \sum_i \sigma_i f(x_i,q) \le \sqrt{d} \sum_k  |\alpha_k|\sqrt{k\log^3(k)}
\]
For general $\|q\|_2 \leq R$ we get
\[
...
$$
\sum_i \sigma_i f(x_i,q) = \sum_k \alpha_k \sum_i \sigma_i \langle x_i,q\rangle^k =  \sum_k \alpha_k  \langle  \sum_i \sigma_i x_i^{\otimes k},q^{\otimes k}\rangle \le \sum_k |\alpha_k| \cdot \| \sum_i \sigma_i x_i^{\otimes k}\| \cdot \|q\|_2^k
$$
By Lemma \ref{uc} we can find signs $\sigma$ such that 
$\| \sum_i \sigma_i x_i^{\otimes k}\| \le c\sqrt{d k \log^3(k)}$. Substituting into the above the lemma follows.
\end{proof}

\begin{theorem}
Let $\tilde{f}$ be an analytical one dimension function over $\R$. There exist a radius $R$ such that for $\|q\|_2 \leq R$ the function family of functions $f_q(x) = \tilde{f}(\ip{q,x})$, indexed by $\|q\| \leq R$, has Komlos complexity of $O(\sqrt{d})$ 
\end{theorem}
\begin{proof}
Recall that for analytical functions $f$ we have 
$$\left| \frac{d^k \tilde{f}}{dx^k}(x) \right| / k! \leq C^{k+1} $$
for some constant $C$. Considering the taylor expansion of $\tilde{f}$ near zero we get that for $R < 1/C$, the sum
$$ \sum_k  |\alpha_k| R^k \sqrt{ k\log^3(k)} $$
corresponding to Lemma~\ref{lem:komlos anl} converges to a constant. The result follows.
\end{proof}

\begin{corollary}
The Komlos complexity of the Logistic function $f(\ip{q,x}) = \log(1+\exp(\ip{q,x}))$ in dimension $d$, for $\|q\|_2 \leq 1$ is $O(\sqrt{d})$.
\end{corollary}
