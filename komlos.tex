...
\section{Universal Coresets for Functions of Inner Products}


\begin{lemma}\label{uc}
For any set of vectors $x_i \in \R^d$ there exist a set of signs $\sigma_i$ such that for all $k$ simultaneously $\|\sum_i \sigma_i x_i^{\otimes k}\| \le O(\sqrt{d k\log^{3}{k}})$ (the $3$ power of the term $\log(k)$ can be reduced to any constant power larger than $2$). Here 
$\|X\| = \max_{q, \|q\|=1} \langle X, q^{\otimes k}\rangle$.
...
Here, $\|\cdot\|$ is a special spectral norm defined by 
$$\|\psi\| = \max_{q, \|q\|=1} \langle   \psi_k,q^{\otimes k}\rangle  = \max_{q, \|q\|=1} \sum_{i_1,\ldots,i_k=1}^d \psi_k(i_1,\ldots,i_k)\Pi_{k'=1}^{k}q(i_{k'})$$
%
Since $\|\cdot\|_T$ is a norm the set $\mathcal K  = \{\psi \; | \; \|\psi\|_T \le c\sqrt{d}\}$ is convex. 
...
Finally, note that for $\|x\| \le 1$ we have $\|\psi(x)\| = (\sum_k  1/k\log^2(k))^{1/2} = O(1)$.
...
\begin{lemma}
Let $f$ be a function of $\langle x,q\rangle$ such that $f(\langle x,q\rangle) = \sum_k \alpha_k \langle x,q\rangle^k$ is its Taylor expansion. The Komlos Complexity of the Family of functions $f$ (indexed by $q$) is bouded by
\[
...
$$
\sum_i \sigma_i f(x_i,q) = \sum_k \alpha_k \sum_i \sigma_i \langle x_i,q\rangle^k =  \sum_k \alpha_k  \langle  \sum_i \sigma_i x_i^{\otimes k},q^{\otimes k}\rangle \le \sum_k |\alpha_k| \cdot \| \sum_i \sigma_i x_i^{\otimes k}\|
$$
...
\end{lemma}
\begin{proof}
The sum converged to a constant.
\end{proof}
\begin{lemma}
The Komlos complexity of the Logistic function in dimension $d$ is $O(\sqrt{d})$.
\end{lemma}
\begin{proof}
The sum converged to a constant.
\end{proof}
