...
We provide general techniques for bounding the discrepancy of machine learning problems and specifically do so for matrix covariance approximation, logistic regression, kernel density and any analytic function of the dot product or the squared distance.
...
Standard generalization results show that for a large enough value of $n$ the average approximates the mean if the complexity of $\F$ is bounded and the samples $x_i$ are drawn i.i.d.\ from underlying distribution. We therefore focus on approximating the average, or rather, the sum $\sum_{i=1}^{n} f(x_i)$. For notational convenience we use a parameter $q \in \mathcal Q$ to index into $\F$ explicitly $F(q) = \sum_{i=1}^{n} f(x_i, q)$ \zk{It seems confusing that we are moving from $f\in \F$ to a universal single function $f$, $f(q,x)$. I'm thinking of calling it $g \in \F$ above and saying the for $\Q \equiv \F$, meaning there is a bijective mapping between them, we have $g(x)=f(q,x)$ for all $x$, for any matching pair $f,q$}. 
...
\]
\end{definition}
The discrepancy of $f$ or $\F$ without a reference a set $\{x_1,\ldots,x_m\}$ is the upper bound on any subset of $\X$ of size $m$.
...
The set $A$ is the set of possible induces losses. Namely, $a\in A$ if there is a model $q$ such $a_i = f(x_i,q)$.
The equivalence between the definitions is therefore straight forward. 
...
This set is, in fact, a coreset. The discrepancy of a problem helps us determine the obtainable coreset size. 
...
Now consider $\tilde F_{+}(q) = F(q) + E(q)   = \sum_{i | \sigma_i=1} 2 f(x_i,q)$ and similarly $\tilde F_{-}(q) = F(q) - E(q)  =  \sum_{i | \sigma_i=-1} 2 f(x_i,q)$. We have that both $\tilde F_{+}(q)$ and $\tilde F_{-}(q)$ are approximations for $F(q)$ obtained by coresets of item weights of $2$ and an error at most $|\tilde F_{\pm}(q)- F(q)| =  |E(q)|$.
...
Surprisingly, the space complexity of the streaming algorithms is only poly-logarithmically larger than the best offline coreset.
This section assumes access to an algorithm for finding low discrepancy signs $\sigma$.
...
For any function family $\F$ with a corresponding discrepancy $K_m = O(c/\sqrt{m})$ there exists a fully-mergeable streaming coreset randomized algorithm of size $O\left(c^2\log^3\log(1/\delta) /\eps^2\right)$ \el{$O\left(c^2\log^3\log(n?/\delta) /\eps^2\right)$} \zk{its not a typo - we only need the dependence on $n$ in the case of $K_m=c/m$} whose error for any fixed function $f \in \F$ is at most $\eps n$ with probability at least $1-\delta$. 
\end{theorem}
