...
We are now ready for the case where $K_k = \min\{k, d\sqrt{k}\}$. We set $k_h=2$, resulting in $K_h=2$, for all $h \leq H'$ and obtain
\begin{align*}
...
$$ \text{Err}(q) = n \cdot O\left( \frac{\log \log(n/\delta)d + 1}{\sqrt{k}}  \right) =  n \cdot O\left( \frac{\log \log(n/\delta)d }{\sqrt{k}}  \right) $$
...
$$k \approx \frac{d^2 \log^2\log(n/\delta)}{\eps^2}$$ 
we get a bound of $\eps n$ for the error and a memory usage of 
$$ O\left( \frac{d^2 \log^3\log(n/\delta)}{\eps^2}\right) $$
\end{proof}
