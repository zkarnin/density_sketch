...
\item Kernel density estimation. The state-of-the-art is by \cite{DBLP:journals/corr/abs-1802-01751} giving a $\sqrt{d\log(1/\eps)}/\eps$ coreset for kernel density, for Lipchitz psd kernels. The result is constructive though it is polynomial rather than (quasi-)linear in the data size. The authors give an almost matching lower bound of $\sqrt{d}/\eps$ and pose an open question for closing the gap between the bounds.
