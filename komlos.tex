...
Consider the function $F = \sum_{i=1}^{m} f(x_i,q)$ and the error function $E(q) = \sum_{i=1}^{m} \sigma_i f(x_i,q)$ where $\sigma_i \in \{-1,1\}$.
Now consider $\tilde F(q) = F(q) + E(q) =  \sum_{i | \sigma_i=1} 2f(x_i,q)$ 
or alternatively $\tilde F(q) = F(q) - E(q) = \sum_{i | \sigma_i=-1} 2f(x_i,q)$. We have that $\tilde F(q)$ is an approximation for $F(q)$ obtained by a coreset of item weights of $2$ and an error at most $|F_1(q) - F(q)| =  |E(q)|$.
%
...
\end{fact}
\begin{fact}
Let $\F$ be a function family with a corresponding Komlos Complexity $K_m(\F) = O(c/\sqrt{m})$. Let $F(q) = \sum_{i=1}^{n}f_i(q)$ where $f_i \in \F$. There exists a coreset for $F$ of size $k = O(c/\eps^2)$ whose error is at most $\eps n$.
\end{fact}
...
Then, repeat the process and create a coreset of size $n/4$ of items of weight $4$. Here, you incur error of $2c$ or $2c\sqrt{n/2}  = \sqrt{2n}$.
Note that the sum of errors is asymptotically dominated by last iteration. 
...
This assumes the existence of an algorithm for finding low discrepancy signs $\sigma$.
A trivial way to obtain such a construction is known as the merge-reduce trick \el{\cite{??} Har Peled?}.
\el{What do we get from merge-reduce here? $\log(n)/\eps^2$ or $\log^2(n)/\eps^2$? Is it like MRL?)}
The following theorems claim that a better construction is also possible.

%
%of function family $\F$ is low, 
%and one has an 
%
...
For any function $f$ with a corresponding Komlos Complexity $K_m = O(c/m)$ there exists an fully-mergeable streaming coreset algorithms of size 
...
For any function $f$ with a corresponding Komlos Complexity of $K_m = O(c/\sqrt{m})$ there exists an fully-mergeable streaming coreset algorithms of size 
$$O\left(c\log^3\left(\min(\eps^2 n/c, \log(n/\delta)) \right)/\eps^2\right)$$ 
whose error is at most $\eps n$ and failure probability at most $\delta$. For $\delta=0$ this results in a deterministic algorithm. The algorithm must be given $\eps, \delta$ as input and does not require a priori knowledge of the stream length $n$.
\end{theorem}
...
\]
This upper bound allows us to find values for $\sigma_i$ which bound $K_m$ using a simple algorithm.
...
Using the framework above this provides a deterministic coreset construction for any positive kernel density estimation of size $O(1/\eps^2)$ such that $\forall \;q\;\; |\tilde F(q) - F(q)| \le \eps n$. This matches, and greatly simplifies, the results achieved by \cite{DBLP:conf/soda/PhillipsT18} and \cite{DBLP:journals/corr/abs-1802-01751}.
