...
Specifically, $f \in \F$ if there exists $q \in \Q$ such that $f(x) = f(x,q)$. 
...
For any function family $\F$ with a corresponding discrepancy $K_m = O(c/\sqrt{m})$ there exists a fully-mergeable streaming coreset randomized algorithm of size $O\left(c^2\log^3\log(1/\delta) /\eps^2\right)$ \el{$O\left(c^2\log^3\log(n?/\delta) /\eps^2\right)$}whose error for any fixed function $f \in \F$ is at most $\eps n$ with probability at least $1-\delta$. 
\end{theorem}
...
In such cases, the algorithm algorithm above performs much better than the worst-case bound predicts. We leave it to future work to define properties of the data that ensure better guarantees by our algorithm.
...
Each compactor contributes at most $w_h n_h K_m \le n K_m$ error. Moreover since $n_h \le n/2^h$ \el{and compactor number $H-1$ observed at least $m$ items we know that $H \le \floor{log_2(n/m)}+1$. TODO: Not sure this is going in the right direction...}
