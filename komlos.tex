...
Second, an extremely simple algorithm for finding coresets (low discrepancy sequences) for any positive kernel density estimation problem. 
...
Standard generalization results show that for a large enough value of $n$ the average approximates the mean if the complexity of $\F$ is bounded and the samples $x_i$ are i.i.d.\ drawn from underlying distribution. We therefore focus on approximating the average, or rather, the sum $\sum_{i=1}^{n} f(x_i)$. For notational convenience we use a parameter $q \in \mathcal Q$ to index into $\F$ explicitly $F(q) = \sum_{i=1}^{n} f(x_i, q)$ . One should think about $q$ as either the model parameters or a query for the sketch. 
...
\[
K_m(A)  = \min_\sigma \max_{a\in A} \frac{1}{m}|\sum_i  \sigma_i  a_i| 
\]
\end{definition}
%
\begin{definition}
...
\[
K_m(f) =  \frac{1}{m}\min_{\sigma} \max_{q \in \Q}  \left|\sum_{i=1}^{m}\sigma_i f(x_i,q)\right|
\]
\end{definition}
%
\begin{definition}
...
The discrepancy of $\F$ w.r.t.\ $\{x_1,\ldots,x_m\} \subset \X$ is 
\[
K_m(\F) =  \frac{1}{m}\min_{\sigma} \max_{f \in \F}  \left|\sum_{i=1}^{m}\sigma_i f(x_i)\right|
\]
\end{definition}
...
For any function family $\F$ with a corresponding discrepancy $K_m = O(c/m)$ there exists an fully-mergeable streaming coreset algorithm of size 
$$O\left(c\log^2\left(\min(\eps n/c, \log(n/\delta)) \right)/\eps\right)$$ 
whose error is at most $\eps n$ with probability at least $1-\delta$. 
\end{theorem}

\begin{theorem} \label{thm:streaming2}
For any function family $\F$ with a corresponding discrepancy $K_m = O(c/\sqrt{m})$ there exists an unbiased fully-mergeable streaming coreset algorithm of size 
$$O\left(c^2\log^3\left(\min(\eps^2 n/c, \log(n/\delta)) \right)/\eps^2\right)$$ 
whose error is at most $\eps n$ with probability at least $1-\delta$.
\end{theorem}
For both statements above, setting $\delta=0$ yields a deterministic algorithm. The algorithms must be given $\eps, \delta$ as input but do not need to know the length of the stream, $n$, in advance. 
...
In the last section we argued that both $F_1$ and $F_{-1}$ are good approximations for $F$. We can also take $F'$ which is $F_1$ or $F_{-1}$ equally likely. Clearly $|F' - F| \le |E|$ as before. But now, $\E[F'] = F$ as well. In the streaming algorithm, we apply this compaction (converting $F$ to $F'$) many times which allows us to use standard concentration results to bound the overall error. The main departure from \cite{DBLP:conf/focs/KarninLL16} is that $F'$ no longer has exactly $1/2$ the size $F$. In the general setting, this only true in expectation.
