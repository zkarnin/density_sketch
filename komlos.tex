...
Naturally, one could iterate this process. Starting with $n$ items and ending with $k$.
Let $F_t$ denote the (unweighted) sum of functions $f$ after $t$ iterations and $n_t$ denote the cardinality of the coreset\footnote{We use the term $a=b \pm c$ to mention $|a-b| \leq c$ }. 
\[
...
$$\sum_{t=0}^{T-1} 2^t n_t D_{n_t} \le n \sum_{t=0}^{T-1} D_{n/2^t} = n\cdot O(D_{n_T}) . $$ 
Setting $n_T$ in a way that $D_{n_T} = O(\eps)$ leads to a coreset with item weights of $w^T$ of the appropriate cardinality and completes the proof.
