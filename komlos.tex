...
\end{theorem}
\begin{proof}
By transforming $x$ to $\tilde{x} = (x, \|x\|^2, 1)$ and $q$ to $\tilde{q} = (-2q, 1, \|q\|^2)$ we get $\ip{\tilde{x},\tilde{q}} = \|x-q\|^2$. Moreover, $\|q\| \le \sqrt{R}$ gives $\|\tilde q\| \le R$. The result follows from applying Theorem~\ref{analitic1} to $f(\ip{ \tilde q, \tilde x}) = f(\|q-x\|^2)$ apply for functions of $\|q-x\|^2$.
\end{proof}
