...
%Let's discuss what this means. For a logistic regression problem, by observing the weights we can change the signs of the inputs so that the logistic loss is always $\log(1+\exp(\ip{q,x}))$, and we can now convert the discrepancy result into a coreset of size $\sqrt{d\log(1/\eps)}/\eps$, as opposed to a random sample of $1/\eps^2$. This has a limitation where we need the labels to determine the coreset. We now proceed to see how we can remove that need.
%
%We will require an assumption regarding the labels of the data: We assume that for some $q \in Q$, there exist at most $\rho n$ data point label pairs $(x,y) \in X \times \{-1,1\}$ such that $\text{sign}(\ip{q,x}) \neq y$. 
