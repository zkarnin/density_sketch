...
Notice that for any $X \neq 0$, $\ip{X, q^{\otimes}}$ is a non-zero polynomial in $q$. It follows that there must be $q$ for which it's value is non-zero, meaning that $\|X\|_{T_k}=0$ iff $X=0$. For a scalar $a$, we clearly have by definition that
$$ \|aX\|_{T_k} = |a|\|X\|_{T_k} $$
Lastly, by the max definition we clearly have
$$ \|X+Y\|_{T_k} =  \max_{q \in \R^d, \|q\|=1} \left| \langle X+Y, q^{\otimes k}\rangle \right| \leq $$
$$\max_{q \in \R^d, \|q\|=1} \left| \langle X, q^{\otimes k}\rangle \right| + \max_{q \in \R^d, \|q\|=1} \left| \langle Y, q^{\otimes k}\rangle \right| = \|X\|_{T_k} + \|Y\|_{T_k}$$
\end{proof}
