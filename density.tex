...
\item in matrix row subset selection we have $f(x, q) = \langle x,q \rangle ^2$.
...
It is clear that we can get $\|\sum_i s_i x_i x_i^T\| \le \sqrt{k \log(d)}$. Could it would see that maybe $O(1)$ is also possible?
...
If so, it would give a covariance sketch is subset selection of size $1/\eps$ which would be an improvement over frequent directions.
