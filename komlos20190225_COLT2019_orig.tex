...
%In this talk we ask the question, how much data do you really need? To answer that, we explore the notion of coresets in machine learning. Those are small sets of data points (or examples) with the following unique property.  For all models, the loss suffered on the coreset is roughly the same as the loss on the entire data. As a result, optimizing (training) over the coreset is simultaneously more efficient and provably close to optimal. Much research was devoted to finding small coresets for logistic regression, linear regression, clustering, density estimation, covariance approximation and many other problems. This talk will present a new measure of function class complexity, namely, class discrepancy. Class discrepancy captures the coreset complexity (minimal coreset size) for all the above problems and many others. These coresets are asymptotically smaller than those currently known and are often provably optimal. In fact, there exist mergeable streaming coreset constructions for all machine learning problems with bounded class discrepancy. This, information theoretically, allows for communication free, distributed, order agnostic, streaming machine learning. 
\end{abstract}
...
%That is the usefulness of coresets in the context of machine learning. 
...
These include Logistic Regression $F(q) =  \sum_i \log(1+\exp(\ip{y_ix_i,q}))$, Covariance or matrix approximation  $F(q) =  \sum_i \ip{x_i,q}^2$, sigmoid activation loss $F(q) =  \sum_i 1/(1+\exp(\ip{y_ix_i,q}))$, linear regression $F(q) =  \sum_i (\ip{y_ix_i,q} -y_i)^2$ and many others. For all the aforementioned  $x,q \in R^d$ and $y_i \in \{-1,1\}$.
By bounding the class discrepancy of all such functions we obtain coresets of size $O(\sqrt{d}/\eps)$ for all of them.

We note that while we obtain universal {\it additive guarantee} it is often much harder to get a  {\it multiplicative guarantee}
...
They provide a lower bound for the size of a multiplicative error coreset
proving in particular that in general it is not possible to achieve $m \ll n$. 
%They show that in general it is not possible to achieve $m \ll n$. 
...
Approximation here means that $|\tilde F(q)  - F(q)| \le \eps n$ for all $q \in \mathcal Q$ simultaneously. % or for every fixed $q$ with probability at least $1-\delta$. 
...
The size of the coresets appear to be intimately tied to the class discrepancy properties of the associated functions.
...
Specifically, every element of $\F$ can be written as $f_q$ and has a unique $q \in \Q$ such that $f_q(x) = f(x,q)$. 
...
The connection is a simple application of the folklore argument known as the ``the halving trick".
...
In fact, we show that for any positive kernel $D_m = O(1/\sqrt{m})$. This bound is superior to that of the previous section for high dimensions $d > m$. More importantly though, there is a very simple, intuitive, and deterministic algorithm for computing the signs $\sigma$. 
